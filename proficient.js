...
// utils
    ...
    utils.toUint8 = function (value) {
        if (utils.isTypedArray(value) || Array.isArray(value) || value instanceof ArrayBuffer || typeof value === "number") {
            value = new Uint8Array(value);
        } else if (typeof value === "string") {
            value = new Uint8Array(utils.getCodesFromString(value));
        } else {
    ...
    utils.toBuffer = function (buffer) {
        return utils.toUint8(buffer).buffer;
    };
    ...
    function mediaConnection(provider, peer, options) {
        ...
        mc.descriptions = function (type, set) {
            ...
            if (type === "local") {
                ...
                description = store.setLocalDescription;
            } else if (type === "remote") {
                if (typeof set === "boolean") {
                    store.setRemoteDescription = set;
                }
                description = store.setRemoteDescription;
            }
            return description;
        };
        ...
        dc.send = (function () {
            ...
            function sendMessage(data, metadata) {
                var type = typeof data === "string"
                    ? 1
                    : 2;
                var chunksize = utils.chunksize;
                var size = data.size || data.byteLength || data.length;
                var total = Math.ceil(size / chunksize);
                var start = 0;
                var end = start + chunksize < size
                    ? start + chunksize
                    : size;
                var count = 1;
                var id = gid();
...
// peer
    function getPeer(user, room, options) {
        ...
        pro.chat = function (peer, options) {
            var connection;
            var members = pro.socket.members();
        ...
        pro.gotCandidate = function (data, peer) {
            ...
            var connection = pro.getConnection(peer, payload.id);
